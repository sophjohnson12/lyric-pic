...
export async function getLyricGroupMembers(groupId: number): Promise<AdminLyricGroupMemberRow[]> {
  const { data, error } = await supabase
    .from('lyric')
    .select('id, root_word, is_blocklisted, is_flagged, stem')
...
export async function updateAppConfig(
  updates: Partial<Pick<AppConfig, 'theme_primary_color' | 'theme_secondary_color' | 'theme_background_color' | 'enable_images' | 'enable_lyric_flag' | 'enable_image_flag' | 'enable_backgrounds' | 'enable_map' | 'min_image_count' | 'max_image_count' | 'max_guess_count' | 'min_song_lyric_count' | 'top_distinctive_count' | 'max_distinctive_value'>>
): Promise<void> {
  const { error } = await supabase
    .from('app_config')
