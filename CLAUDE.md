...
**Map page landmark reveal** (`src/components/game/MapPage.tsx` + `src/components/game/RevealLandmarkModal.tsx`): Elements with a `song_id` are visually locked (black + padlock) until revealed. A "Reveal Next Landmark" button (fixed bottom-center, `bg-primary`) appears when at least one eligible element exists. An element is **eligible** when its specific `song_id` appears in the player's played-songs localStorage (only level-specific keys `lyricpic_played_songs_<slug>_level_<levelSlug>` are read — the legacy key without `_level_*` is intentionally ignored). `RevealLandmarkModal` is a mini-game: shows the song name/album, presents up to 3 locked-element images as choices, wrong guess → WAAPI shake + gray-out, correct guess → WAAPI grow → `onReveal(id)` adds to `lyricpic_revealed_landmarks_<slug>` and smooth-scrolls the map to the element. Revealed elements transition from `brightness(0%)` to `brightness(120%)` via `transition: filter 0.6s ease`. `ChoiceCard` uses `forwardRef` + `useImperativeHandle` to expose `shakeError()` / `growCorrect()` as Promise-returning WAAPI handles. The map is full-width on `md+` (height follows aspect ratio, scrolls vertically) and `w-[300vw]` on mobile (scrolls horizontally).
