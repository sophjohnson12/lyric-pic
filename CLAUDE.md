...
**Map page landmark reveal** (`src/components/game/MapPage.tsx` + `src/components/game/MapLandmarkModal.tsx`): Elements with a `song_id` are visually locked (black + padlock) until revealed. A "Place a Landmark" button (fixed bottom-center, `bg-primary`) appears when at least one eligible element exists. An element is **eligible** when its specific `song_id` appears in the player's played-songs localStorage (only level-specific keys `lyricpic_played_songs_<slug>_level_<levelSlug>` are read — the legacy key without `_level_*` is intentionally ignored). `MapLandmarkModal` is a mini-game: shows the song name/album, presents up to 3 locked-element images as choices, wrong guess → WAAPI shake + distractor grays out (solid `bg-neutral-300` container, `opacity-50` on `<img>` only). When all distractors are eliminated, the correct card auto-triggers. Correct guess flow: (1) distractors gray out instantly; (2) if correct card is not in center position, it swaps with the center card via WAAPI `translateX` (350ms, `fill:'forwards'`); (3) map smooth-scrolls + CSS-scales to 1.5× toward the landmark (`handleRevealStart`, ~850ms) while the modal stays fully visible; (4) a fixed-position overlay `<img>` appears at the card's visual position, grows to 1.5× (300ms WAAPI), then shrinks via `translate+scale` to land precisely on the map landmark (600ms WAAPI); (5) only when the overlay fully lands does `setRevealedIds` fire — the silhouette transitions `brightness(0%) → brightness(120%)` via `transition: filter 0.6s ease`; (6) map scale resets to 1 after 600ms. `ChoiceCard` uses `forwardRef` + `useImperativeHandle` to expose `shakeError()`, `slideTo(dx)`, `getDivRect()`, and `getImageRect()` as Promise-returning WAAPI handles. The map content div has `margin: '50vh 50vw'` (not a padding wrapper) so edge landmarks can always be scrolled to viewport center. The map is full-width on `md+` (height follows aspect ratio, scrolls vertically) and `w-[300vw]` on mobile (scrolls horizontally).
